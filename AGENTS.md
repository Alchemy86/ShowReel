...
| A `Music` track's `arrangement: Vec<Section>` gives it structure over time — named sections (a bar length, an optional tempo override, an `Intensity` of which voices play and how densely) walked by `Music::plan`, the one function both `render_samples` and `manifest` use so the audio and its timings can never drift apart. An empty arrangement (the default) reproduces the old single-intensity tune exactly — capability added, not a mode to opt into. `Intensity` mirrors `Mood`'s bare-word-or-object shorthand (`"kick"`/`"pulse"`/`"build"`/`"full"`/`"silence"`, or a per-voice object); an arrangement shorter than the requested duration loops. `Music::manifest` emits beat/bar/section timings — nested the same way `NarrationManifest` nests lines→words — so a shot (a flashing terminal cursor, a logo drop, narration starting on the bar) can cue off exact times instead of a hand-timed guess. `showreel music` is the standalone CLI export: a WAV plus its `<name>.beats.json` manifest, built through the same word-or-object JSON a film's `"music"` block accepts (`serde_json::Value` into `Music`'s own `Deserialize`), not a second parser. `examples/pokemon_opener/*.arrangement.json` are three original title-screen opener arrangements (not a transcription of any specific game's theme) proving the feature | `src/music.rs` (`Section`, `Intensity`, `VoiceLevel`, `Music::plan`/`manifest`/`natural_duration`), `src/bin/showreel.rs` (`cmd_music`) |
...
- **`examples/pokemon_opener/*.arrangement.json` are the proof arrangements for
  ...
  tempo; density alone carries the "slow and lonely, then it lifts" arc — a
  ~22 dB mean-level jump measured intro-to-theme), `tempo-lift` (the intro
  section carries its own slower `bpm`, a real tempo lift into the theme, not
  just a density change), `call-and-answer` (two short calls each followed by a
  bar of true `"silence"` before the build). No rendered audio is committed
  ...
  <file>` regenerates the WAV+manifest on demand.
