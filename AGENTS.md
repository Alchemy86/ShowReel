...
| A segmented render's segments fan out across a **bounded, machine-aware worker pool**, not strictly one at a time — `src/budget.rs`, sized from a *measured* per-worker memory cost (the first pending segment always renders serially first, sampling system-wide `/proc/meminfo` availability — not `showreel`'s own RSS, which misses the `ffmpeg` children that are most of a render's real footprint — tightened by the enclosing cgroup's own `memory.max`/`memory.current` when there is one, since a `systemd-run`/container memory cap changes nothing `/proc/meminfo` reports), the machine's real core count, and a small cross-process JSON ledger (advisory-locked via `std::fs::File::lock`, under the system temp dir) so a *second* concurrent `showreel render` sees what the first already claimed. A worker gets its **own** `AssetStore`, not the caller's shared one: a streaming clip's decoder is one `ffmpeg` child behind one mutex (`src/assets/clip.rs`), genuinely sequential by design, so concurrent workers sharing it would thrash rather than parallelise; each worker's own store means its own independent decoder and the real, measured cost the budget plans against. **`ffmpeg`'s own decode and encode threads must be capped per worker** (`-threads`, a thread-local hint in `clip.rs` for decode, `EncodeOptions.threads` for encode) or concurrency is a *regression*, not a speedup — measured directly: four unthrottled workers on a 20-core box ran slower than one serial pass (15x the involuntary context switches; ffmpeg defaults to using every core it can see for both decode and encode, and N workers each doing that oversubscribes by roughly Nx). This is why encoded *bytes* are not guaranteed identical between concurrency levels (x264's own bitstream is sensitive to thread count — the same documented trade already made for segmenting itself, see the GIF/segmented-rendering row above) even though decoded *pixels* are proven identical (`src/segments.rs`'s own test decodes and compares frames, not raw bytes). Never refuses to render — degrades toward serial, printing why, all the way down to one worker if headroom is that tight; a worker also stops picking up further segments mid-render if headroom turns critical. See `docs/render-budget.md` for the full incident, the measured 2.13x real-CLI speedup, and the constrained-memory (`systemd-run --scope -p MemoryMax=`) proof that it degrades instead of getting OOM-killed | `src/budget.rs`, `src/segments.rs` (`render_segmented`), `src/assets/mod.rs` (`AssetStore::roots`) |
| The mobile cut's frame rate is a **choice**, not a hardcoded 30 — `--mobile-fps <N>` on the CLI, defaulting to the film's own fps rather than downsampling motion away (a Game Boy walk cycle read as a hop at a flat 30fps on anything animated faster). The library's own `MobileOptions::default()` is unchanged (still 30, still what `studio`/`mcp` use — neither has a film's own fps to default against the way the CLI does): 30fps was a real, documented delivery constraint (Telegram's `sendVideo` rejects 60fps outright), not an arbitrary choice, so `--mobile-fps 30` is how to restore it explicitly rather than something the default should silently keep doing to every film | `src/encode.rs` (`MobileOptions`), `src/bin/showreel.rs` (`cmd_render`) |
...
| Generated music (`Audio.music: Option<Music>`) is a *source*, not a parallel audio path: a `Music` spec resolves to a synthesised WAV exactly where a file track's `asset` resolves to a path (`Film::resolve_audio_tracks`), so from `Audio::resolve` on it is an ordinary `AudioInput` — fades/gain/mixing/mobile compose for free. One idiom (the NES/SID chiptune the devlog's `chiptune.py` proved, ported faithfully — envelope/spectrum correlation ≈0.9997/0.9999 vs the reference), not a synthesiser: a `Mood` is *data* (a progression + step patterns; `funk`/`dreamy`/`title`). Authoring mirrors `Grade`'s bare-word-or-object shorthand. Deterministic (a seeded `splitmix64`, unlike the reference's `numpy` global RNG) so renders reproduce and the temp WAV can be content-addressed. `MusicFit::Film` nudges the tempo so a whole number of bars spans the film (ends on a downbeat); the general per-cut-time solver is a documented next step, not half-built | `src/music.rs`, `src/audio.rs` (`Audio::music`), `src/timeline.rs` (`resolve_audio_tracks`, `resolve_music`) |
| A `Music` track's `arrangement: Vec<Section>` gives it structure over time — named sections (a bar length, an optional tempo override, an `Intensity` of which voices play and how densely) walked by `Music::plan`, the one function both `render_samples` and `manifest` use so the audio and its timings can never drift apart. An empty arrangement (the default) reproduces the old single-intensity tune exactly — capability added, not a mode to opt into. `Intensity` mirrors `Mood`'s bare-word-or-object shorthand (`"silence"`/`"tone"`/`"kick"`/`"pulse"`/`"build"`/`"full"`, or a per-voice object); an arrangement shorter than the requested duration loops. **Open an opener on `"tone"`, not `"kick"`/`"pulse"`**: the drums are noise-burst and unpitched by design (top-of-file module docs), so a kick/pulse-only section is a lonely *percussive* beat with zero tonal content — right for a rhythm building toward a tune, not for the first thing an opener sounds (a real captain-rejection: "sounds like a hard slap, not a beat"). `VoiceLevel::Held` (bass/lead only; silent on a drum, same as `Off`) triggers once a bar like `Sparse` but is sounded through `held_tone` rather than `pulse` — a soft attack then an exponential decay to ~1/8 peak across the whole bar, ringing and fading the way a struck note does, not `pulse`'s flat-sustain-then-cutoff (which either drones or clicks on a held note). `Intensity::TONE` = `{bass: Held, lead: Held, kick: Off, hat: Off}`. `Music::manifest` emits beat/bar/section timings — nested the same way `NarrationManifest` nests lines→words — so a shot (a flashing terminal cursor, a logo drop, narration starting on the bar) can cue off exact times instead of a hand-timed guess; timings are identical regardless of which `Intensity` a section uses, since voice choice never touches `plan`'s bar grid. `showreel music` is the standalone CLI export: a WAV plus its `<name>.beats.json` manifest, built through the same word-or-object JSON a film's `"music"` block accepts (`serde_json::Value` into `Music`'s own `Deserialize`), not a second parser. `examples/pokemon_opener/*.arrangement.json` are three original title-screen opener arrangements (not a transcription of any specific game's theme) proving the feature | `src/music.rs` (`Section`, `Intensity`, `VoiceLevel`, `held_tone`, `Music::plan`/`manifest`/`natural_duration`), `src/bin/showreel.rs` (`cmd_music`) |
...
| Audio isolation (`showreel isolate`) splits a video/audio file's soundtrack into a speech stem and a music/effects stem, and can mux a chosen stem back against the original picture. Same shape as narration's Kokoro bake — a subprocess boundary to a pinned Python venv — but shells out to each model's own CLI directly rather than an embedded driver script, since there is no bespoke logic (no word timings, no pause direction) to keep testable independent of a model. **Model choice is a licence decision, not a quality one**: `spleeter` (default) is the only one of the three checked (Spleeter/Demucs/Open-Unmix) whose pretrained weights, not just its code, are actually MIT — Demucs's `htdemucs` weights are MIT-code-but-research-only-weights (the maintainer's own words, `docs/audio-isolation.md`), the same code/weight split that ruled out XTTS-v2 before. `--model demucs` is offered anyway (measurably better separation, real precedent in `data/voice-clone-qwert`) but warns loudly on every use rather than shipping the restriction quietly | `src/isolate.rs`, `docs/audio-isolation.md` |
...
- **A `systemd-run --scope -p MemoryMax=`/container memory cap is invisible to
  `/proc/meminfo`** — that file always reports the *host's* memory, not a cgroup's
  tighter ceiling. `src/budget.rs`'s `MachineState::probe`/`available_bytes_now` also
  read this process's own cgroup v2 leaf (`/proc/self/cgroup`'s `0::` line ->
  `/sys/fs/cgroup<that path>/memory.max`/`memory.current`) and take whichever of the
  two readings is tighter. Skip this and a render inside a real memory-constrained
  environment (a CI container, a `systemd-run` cap — exactly how this feature's own
  memory-ceiling claim was verified) would plan against memory it does not actually
  have and get OOM-killed by the cgroup regardless of what the budget decided.
- **Concurrent segment workers must cap `ffmpeg`'s own `-threads`, on both the decode
  and encode side, or concurrency is a measured *regression*.** `ffmpeg` defaults to
  using every core it can see for both; N concurrent workers each left at that default
  oversubscribes the machine by roughly Nx on top of `rayon`'s own raster-thread pool.
  Found by direct measurement, not by reasoning about it: the first working version
  (decode/encode threads uncapped) ran a 1920x1080/96s film in 102s against a 47.7s
  serial baseline — slower, with lower CPU utilisation and 2.4x the involuntary context
  switches. See `docs/render-budget.md`'s "the naive version was slower" section before
  touching `src/segments.rs`'s thread-count math.
...
- **`examples/pokemon_opener/*.arrangement.json` are the proof arrangements for
  `Music.arrangement`** — three original title-screen opener structures (a
  progression, not a transcription of any specific game's theme), each `showreel
  music --arrangement <file> ...`-able standalone. `lonely-pulse` (one constant
  tempo; density alone carries the "slow and lonely, then it lifts" arc), `tempo-lift`
  (the intro section carries its own slower `bpm`, a real tempo lift into the theme,
  not just a density change), `call-and-answer` (two short calls each followed by a
  bar of true `"silence"` before the build; `call-2` layers a light `hat: "sparse"`
  onto its held tone for a second-time-round lift). No rendered audio is committed
  here — unlike the other proof films, these are pure JSON source with nothing
  to regenerate *from*; `showreel music --mood title --bpm <n> --arrangement
  <file>` regenerates the WAV+manifest on demand. **Their intro/call sections open
  on `"tone"` (a held, ringing note), not `"kick"`/`"pulse"`** — the first version
  opened on a kick-only section and was rejected on listen ("sounds like a hard
  slap, not a beat"): the drums are unpitched noise bursts, so a kick-only intro
  carried no tonal content at all. `VoiceLevel::Held`/`Intensity::TONE` and
  `held_tone`'s decay envelope exist specifically to fix this — see the `arrangement`
  row in the table above. Judge any further change to these by ear, not by a
  metric; a held note that sounds thin or clicks off is the same class of failure.
...
- **`showreel isolate` needs its own venv per model, and each has a real
  install gotcha.** Spleeter pins Python `<3.12` (TensorFlow 2.12.1); Demucs
  wants current PyTorch on 3.12 — one shared venv would pin one against the
  other. Spleeter, left alone, downloads its pretrained weights to a
  *relative* `pretrained_models/` in whatever directory `showreel` happens to
  be invoked from (its `MODEL_PATH` env var default) — it landed in this
  repo's own root once before `run_separation` started pinning `MODEL_PATH`
  to the venv's own directory. Demucs's `pip install demucs` does **not**
  pull in `numpy`/`soundfile` on its own — `import demucs.pretrained` fails
  with `ModuleNotFoundError: No module named 'numpy'` right after a clean
  install; both need installing alongside it explicitly. See
  `docs/audio-isolation.md`'s setup section.
